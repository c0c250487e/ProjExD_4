...
class EMP:
    ...
    def __init__(self, emys: pg.sprite.Group, bombs: pg.sprite.Group, screen: pg.Surface):
        ...
        for bomb in bombs:
            bomb.speed /= 2  # 速度半減
            bomb.state = "inactive"  # 状態をinactiveに
        
        # 見た目：黄色の矩形を表示
        self.overlay = pg.Surface((WIDTH, HEIGHT))
        self.overlay.set_alpha(128)  
        self.overlay.fill((255, 255, 0))  

    def update(self):
        """
        EMPの効果時間を更新し、終了したら自身を削除
        """
...
def main():
    ...
    while True:
        ...
        #     if bird.state == "hyper":
        #         # bomb.kill()
        #         # for bomb in pg.sprite.groupcollide(bombs, bird, True, True).keys():  # ビームと衝突した爆弾リスト
        #         exps.add(Explosion(bomb, 50))  # 爆発エフェクト
        #         score.value += 1  # 1点アップ
                        
            # else:
            #     bird.change_img(8, screen)  # 例：やられ画像に変更
            #     pg.display.update()
            #     time.sleep(2)
            #     # print(bird.state)
            #     return  # ゲーム終了
